...
static ssize_t read_fun(struct file *file, char *user_buf, size_t len, loff_t *off)
{
    ...
    for (i = 0; i < data_len / 16; i++)
    {
        ...
        for (j = 0; j < 16; j++) // copy 1 khoi du lieu
            one_data[j] = data[i * 16 + j];

        printk("one data: %s\n", one_data);

        if (strcmp(type, "encrypt") == 0) // ma hoa du lieu
            crypto_cipher_encrypt_one(tfm, one_cipher, one_data);
        if (strcmp(type, "decrypt") == 0) // giai ma du lieu
            crypto_cipher_decrypt_one(tfm, one_cipher, one_data);
        for (j = 0; j < 16; j++) // copy vao ban cuoi cung
...
static ssize_t write_fun(struct file *file, const char *user_buff, size_t len, loff_t *off)
{
    ...
    if (strlen(hex_data) % 32 == 0)  // add padding cho data
...
static int md_init(void)
{
    ...
    tfm = crypto_alloc_cipher("aes", 0, 0); // khoi tao bo ma hoa
    crypto_cipher_setkey(tfm, key, 16); // set ket bo ma hoa

    alloc_chrdev_region(&dev_num, 0, 1, "tenthietbi"); // tao driver
    device_class = class_create(THIS_MODULE, "class"); // tao class cho driver
    device_create(device_class, NULL, dev_num, NULL, "aes_encrypt"); // tao thiet bi

    kernel_buffer = kmalloc(MEM_SIZE, GFP_KERNEL); // cap phat bo nho cho nhan

    char_device = cdev_alloc();  // cap phat char driver
    cdev_init(char_device, &fops);  // khoi tao 
    cdev_add(char_device, dev_num, 1); // add driver vao he thong
...
{
    crypto_free_cipher(tfm); // free, giai phong bo nho
